...
/** Resolve version from SRC_ROOT's CURRENT_VERSION.txt or local file */
const ver =
  process.env.VER
...
const SRC = process.env.SRC_ROOT
  ? path.resolve(process.env.SRC_ROOT)                                  // e.g. ~/appian-doc-sync/mirror/suite/help/<ver>
  : (process.env.SRC_DIR ? path.resolve(process.env.SRC_DIR, ver)       // e.g. ~/appian-doc-sync/mirror/suite/help + ver
                         : path.join('mirror','suite','help',ver));     // local fallback (unused in your setup)

const OUT = process.env.OUT_DIR || path.join('optimized-md','suite','help',ver);

/** Tunables (env-overridable) */
const SPLIT_OVER_KB   = Number(process.env.SPLIT_OVER_KB || 0);     // 0=off
const CHUNK_TARGET_KB = Number(process.env.CHUNK_TARGET_KB || 200);
const BIG_HTML_KB     = Number(process.env.BIG_HTML_KB || 1200);    // use streaming mode above this size
const SEG_TARGET_KB   = Number(process.env.SEG_TARGET_KB || 300);   // streaming segment target
...
/** Do NOT strip <noscript> (JavaDoc uses it) */
...
function titleFromHTML(html, fallback) {
  ...
  if (t) return t.replace(/[-|·•]\s*Appian.*$/i,'').trim();
  return fallback;
}

function cheapStripHTML(html) {
  // keep <noscript> intact
  return html
...
function convertSnippetToMD(htmlSnippet) {
  ...
  td.use(gfm);
  // remove "latest version" banners inside snippet
...
function convertLargeHTMLToMDChunks(html, targetBytes) {
  ...
  const parts = stripped.split(/(?=<h2\b[^>]*>)/gi); // keep <h2> at segment start
...
function mdH1(md){ return md.match(/^\#\s+(.+)$/m)?.[1]?.trim() || ''; }
function mdH2(md){ return md.match(/^\#\#\s+(.+)$/m)?.[1]?.trim() || ''; }
function baseTitleFromRel(rel){
  const base = path.basename(rel).replace(/--p\d+$/,'').replace(/_/g,' ');
  return base.replace(/\s+/g,' ').trim();
}

function buildFM({srcUrl, rel, pageTitle, md, ver, partIdx=null, partTotal=null}) {
  const title = mdH1(md) || pageTitle || baseTitleFromRel(rel);
  const section = mdH2(md) || undefined;
  const page_id = rel.replace(/--p\d+$/,'');
  const kv = [
    ['source_url', srcUrl],
    ['original_path', rel],
    ['version', String(ver)],
    ...
    partIdx!=null && partTotal!=null ? ['part', `${partIdx}/${partTotal}`] : null,
    ['tags', `["appian","docs","v${ver}"]`]
  ].filter(Boolean);

  const lines = kv.map(([k,v]) => k === 'tags' ? `${k}: ${v}` : `${k}: "${String(v).replace(/"/g,'\\"')}"`);
  return `---\n${lines.join('\n')}\n---\n\n`;
}
...
/** Walk/build list, shard */
fs.mkdirSync(OUT, { recursive: true });
let files = Array.from(htmlFiles(SRC));
if (!files.length) { console.error('No HTML files found at', SRC); process.exit(2); }
if (SHARDS > 1) {
...
for (let i = 0; i < files.length; i++) {
  ...
  try {
    ...
    const pageTitle = titleFromHTML(html, baseTitleFromRel(rel));

    let chunksMD;

    if (isJavaDoc(html, rel)) {
      const nsMatch = html.match(/<noscript[^>]*>([\s\S]*?)<\/noscript>/i);
      if (nsMatch) {
        chunksMD = [convertSnippetToMD(nsMatch[1])];
      } else if ((stat.size/1024) >= BIG_HTML_KB) {
        chunksMD = convertLargeHTMLToMDChunks(html, SEG_TARGET_KB*1024);
        if (!chunksMD.length) chunksMD = [''];
      } else {
        try { chunksMD = [convertFullHTMLToMD(html)]; }
        catch { chunksMD = ['> This page uses JavaDoc frames/scripts that don’t render well offline. See **source_url**.']; }
      }
    } else if ((stat.size/1024) >= BIG_HTML_KB) {
      chunksMD = convertLargeHTMLToMDChunks(html, SEG_TARGET_KB*1024);
      if (!chunksMD.length) chunksMD = [''];
      ...
      chunksMD = overKB ? splitBySize(md, CHUNK_TARGET_KB*1024) : [md];
    }

    fs.mkdirSync(path.dirname(outBase), { recursive: true });
    if (chunksMD.length === 1) {
      let body = chunksMD[0] || '';
      if (body.length < 180) body += `\n\n> Short page. Refer to **source_url** for full context.\n`;
      const fm = buildFM({srcUrl, rel, pageTitle, md: body, ver});
      const out = fm + body;
      fs.writeFileSync(outBase + '.md', out);
      made++; bytes += Buffer.byteLength(out);
    } else {
      chunksMD.forEach((c, idx) => {
        let body = c || '';
        if (body.length < 180) body += `\n\n> Short page segment. See **source_url** for full page.\n`;
        const fm = buildFM({srcUrl, rel, pageTitle, md: body, ver, partIdx: idx+1, partTotal: chunksMD.length});
        const out = fm + body;
        const outPath = `${outBase}--p${String(idx+1).padStart(2,'0')}.md`;
    ...
    try {
      const rel = path.relative(SRC, file).replace(/\\/g,'/');
      const srcUrl = `https://docs.appian.com/suite/help/${ver}/${rel}`;
      const fm = buildFM({srcUrl, rel, pageTitle: baseTitleFromRel(rel), md: '', ver});
      const out = fm + `> Conversion failed; refer to **source_url**.\n`;
      const outBase = path.join(OUT, rel.replace(/\.html?$/i,''));
      fs.mkdirSync(path.dirname(outBase), { recursive: true });
      fs.writeFileSync(outBase + '.md', out);
      made++; bytes += Buffer.byteLength(out);
...
function splitBySize(md, targetBytes) {
  ...
  flush();
  // secondary pass for oversized chunks
  const out = [];
