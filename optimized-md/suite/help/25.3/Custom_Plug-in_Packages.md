...
The following packages are provided by the plug-in container. When creating a [Custom Plug-in](Appian_Plug-ins.html), if you use classes from the packages listed below, you **should not** include the library that contains the class in your plug-in because it may cause classloading conflicts if an instance of the class created by the container is passed to the plug-in.

**Note:**  Of the Appian packages included, only those in [Appian's public Java API](api/) are supported.
...
The following packages are provided by the plug-in container but are deprecated. When creating a [Custom Plug-in](Appian_Plug-ins.html), if you use classes from the packages listed below, you **should** include the library that contains the class in your plug-in so that the plug-in uses the same version of the library regardless of which version of Appian it runs in.
