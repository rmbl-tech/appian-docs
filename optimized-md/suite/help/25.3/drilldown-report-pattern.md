---
source_url: https://docs.appian.com/suite/help/25.3/drilldown-report-pattern.html
original_path: drilldown-report-pattern.html
version: "25.3"
title: "Drilldown Report Pattern"
page_id: "drilldown-report-pattern"
section: "Goal"
...
You can click on these graphs to drill down to detailed information about your data. This is a variation of the [grid with detail pattern](grid-with-detail-pattern.html) Use this pattern if you want to drill into a set of data that is displayed in a grid.

![drilldown_report.png](images/patterns/drilldown_report.png)

## Design structure

The main components in this pattern are [charts](sail/ux-charts.html) and [read-only grids](read-only-grid-configuration.html) that are alternately visible.
...
In this case, **line 138**, the `forEach()` function loops through all unique account status, and a subloop starting on **line 147** then loops through each category (each account name), getting the corresponding value with the `intersection()` function. The [intersection function](fnc_set_intersection.html) is a popular tool for working with charts.
