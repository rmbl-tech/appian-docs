---
source_url: https://docs.appian.com/suite/help/25.3/fnc_crf_customfielddatediff.html
original_path: fnc_crf_customfielddatediff.html
version: "25.3"
title: "a!customFieldDateDiff() Function"
page_id: "fnc_crf_customfielddatediff"
section: "Function"
...
**a!customFieldDateDiff**( _startDate, endDate, interval_ )
...
The `a!customFieldDateDiff()` function can only be used to create a custom record field that evaluates in [real time](custom-record-fields.html#about-custom-record-fields). This means you can reference related record fields, constants, and other [supported functions](#supported-functions) in your calculations.
...
By default, the _startDate_ parameter is subtracted from the _endDate_ parameter. This means that if the _startDate_ is `12/1/2022` and the _endDate_ is `12/10/2022`, the resulting value would be `9` days.

You can use the following values in the _startDate_ and _endDate_ parameters:
...
You can use any of the following supported functions in the _interval_ parameter of `a!customFieldDateDiff()`. This allows you to use functions like `if()` or `not()` to determine which interval value to return.
